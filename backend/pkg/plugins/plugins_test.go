...
func createPlugin(t *testing.T, baseDir string, pluginName string) string {
	...
	err = os.WriteFile(mainJsPath, []byte(""), 0644)
	require.NoError(t, err)
...
func TestHandlePluginEvents(t *testing.T) { //nolint:funlen
	...
	// create main.js and package.json in the sub directory
	err = os.WriteFile(filepath.Join(pluginDirPath, "main.js"), []byte(""), 0644)
	require.NoError(t, err)

	err = os.WriteFile(filepath.Join(pluginDirPath, "package.json"), []byte(""), 0644)
	require.NoError(t, err)
	...
	go plugins.HandlePluginEvents(ctx, "", "", testDirPath, events, ch)
